...
class ScreenSettings(MDScreen):
    ...
    def on_enter(self, *args):
        ...
        Clock.schedule_once(self.open_json_for_name_auto, 1)
    
    def on_leave(self, *args):
        sp = []
        root = self.ids.box_photo
        self.ids.btn_photo.disabled = False
        for child in root.children:
            sp.append(child)
        if len(sp) > 0:   
            self.ids.box_photo.remove_widget(self.img)

    def open_json_for_name_auto(self, *args):
        if exists('main.json'):
            store_read = JsonStore('main.json')
            self.res_marka = store_read.store_get('app')['marka']
            self.res_model = store_read.store_get('app')['model']
            self.key = self.res_marka + self.res_model
...
#################################################3
    def open_closer(self):
        if exists('main.json'):
            ...
            dialog = MDDialog(title=self.text_dialog5)
            dialog.open()
    ...
    @mainthread
    def display(self):
        self.name_file = str(self.path).split('/')[-1]
        self.img.source = self.path
        self.ids.box_photo.add_widget(self.img)
        self.ids.btn_photo.disabled = True
        toast(self.text_dialog4, True, gravity=40)
        ...
        if result:
            ...
            store.put(self.key,name_file=self.name_file)
